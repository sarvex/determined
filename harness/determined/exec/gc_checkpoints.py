...
def delete_checkpoints(
    ...
    for record in to_delete:
        ...
        if not dry_run:
            logging.info(f"Deleting checkpoint {metadata}")
            manager.delete(metadata)
        else:
            logging.info(f"Dry run: deleting checkpoint {metadata.storage_id}")
...
def main(argv: List[str]) -> None:
    ...
    logging.info(f"Using checkpoint storage: {storage_config}")
