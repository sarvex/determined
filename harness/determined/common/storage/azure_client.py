    ...
    def __init__(
        ...
        try:
            ...
            if e.error_code == StorageErrorCode.invalid_uri:  # type: ignore
                logging.warning(
                    f"The storage client raised the following HttpResponseError:\n{e}\nPlease ignore this warning if this is because the account url provided points to a container instead of a storage account; otherwise, it may be necessary to fix your config.yaml."
                )
            else:
